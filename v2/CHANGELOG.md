...
- 新增 `PromptTask[P]`：将润色、翻译、摘要等单轮任务的 system prompt 构造逻辑与常用请求参数（Model / Temperature / MaxTokens / ResponseFormat）绑定为可复用值，调用时按运行时参数 `P` 拼装 system prompt；配套 `Run` 返回纯文本、`RunPromptTaskJSON` 解码为结构化类型，新增 sentinel `ErrNilTaskSystem`。`System` 闭包由调用方实现且不经过 `RunToolLoop` 的注入防护钩子，GoDoc 与 `docs/prompt-injection-defense.md` 新增一节说明该路径的风险与缓解方式
