...
func TestFallbackProviderCustomShouldFallback(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = strict.Chat(t.Context(), &ChatRequest{Messages: []Message{{Role: RoleUser, Content: "hi"}}})
	...
	require.NoError(t, err)
	resp, err := relaxed.Chat(t.Context(), &ChatRequest{Messages: []Message{{Role: RoleUser, Content: "hi"}}})
	require.NoError(t, err)
...
func TestFallbackProviderStopsWhenContextCanceled(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = fallback.Chat(ctx, &ChatRequest{Messages: []Message{{Role: RoleUser, Content: "hi"}}})
	require.Error(t, err)
...
func TestFallbackProviderNestedVendorChains(t *testing.T) {
	...
	resp, err := top.Chat(t.Context(), &ChatRequest{Messages: []Message{{Role: RoleUser, Content: "hi"}}})
	require.NoError(t, err)
