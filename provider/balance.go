...
const (
	...
	// 缓存命中的输入单价通常只有常规输入的一小部分，打散会让这部分收益消失。
	// 会话较长、且上游启用了提示词缓存时优先选它。
	//
	// 哈希按成员权重分配会话，权重大的成员承载更多会话；哈希是确定性的
	// （不含随机种子），多副本部署与进程重启后同一会话仍落到同一成员。
	// 会话键为空的调用退化为平滑加权轮询，故障转移语义与其他策略一致。
	//
	// 该策略必须配合 BalanceOptions.SessionKey 使用：本代码线不提供从 ctx
	// 读取会话标识的默认实现，缺少 SessionKey 时构造即返回
	// ErrInvalidBalanceStrategy，而不是静默退化成普通轮询。
...
type BalanceOptions struct {
	...
	// SessionKey 取出本次调用的会话键，Strategy 为 BalanceSessionAffinity 时必填。
	// 同一会话键的调用会落到同一成员；返回空串表示本次调用无会话归属，
	// 该次调用退化为平滑加权轮询。
	//
	// 典型实现是从 ctx 中取出业务自己的会话标识：
	//
	//	SessionKey: func(ctx context.Context, _ *provider.ChatRequest) string {
	//	    return myapp.ConversationIDFromContext(ctx)
	//	}
...
func NewBalancedProviderWithOptions(members []BalanceMember, opts BalanceOptions) (*BalancedProvider, error) {
	...
	case BalanceSessionAffinity:
		// 无默认会话键来源：缺 SessionKey 就等于粘性完全失效，
		// 与其静默退化成普通轮询，不如在构造期直接拒绝。
		if opts.SessionKey == nil {
			return nil, fmt.Errorf("%w: %q requires BalanceOptions.SessionKey",
				ErrInvalidBalanceStrategy, string(strategy))
		}
	default:
